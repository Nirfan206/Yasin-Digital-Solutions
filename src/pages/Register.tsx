...
const Register = () => {
  ...
  const { register: authRegister, verifyOtp: authVerifyOtp, requestOtp: authRequestOtp } = useAuth(); // Destructure verifyOtp and requestOtp from useAuth
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success && result.otpRequired && result.userId) {
      ...
      // This case should ideally not happen if OTP is always required for register
      navigate('/login');
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-gray-600">
            {otpSent ? 'Enter the OTP sent to your email to complete registration.' : 'Create your account to get started.'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!otpSent ? (
